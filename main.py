...
async def send_message_in_error_channel(message:object):
    """
    Asynchronously sends a message to the error channel identified by channel_id.

    :param message: A string representing the message to be sent.
    :return: The message object that was sent to the channel.
    """     
    me = client.get_user(715325435185201283)
    print(me)
    return me.dm_channel.send(message)



@bot.event
async def on_error(error, message:nextcord.Message):
    from colorama import Fore
    print(f'{Fore.RED}FAIL{Fore.RESET}')
    print('\n \n \n')
    print(f'{Fore.YELLOW}{error}{Fore.RESET}')
    print('\n \n \n \n \n ')
...
@bot.listen()
async def on_thread_create(message:nextcord.Thread):
    
    member_obj = message.guild.get_member(message.owner_id)
    cargos = member_obj.roles # It is a list. Example formatting: <Role id=1036306627818688584 name='Recruiter'>
    channel =  message.parent.id
    if channel == 1020031179493228675:
        print('created thread!')
        for cargo in cargos: 
            if cargo.id == 1036306627818688584:
                return
        role = message.guild.get_role(1036306627818688584)
...
async def on_message(message:nextcord.Message):
    ...
    if message.author != bot.user and message.channel.id != 930099258609258526 and message.channel.id != 1067938880810131517:
